...
zeksApikey = 'Specstor' //ganti pake apikey lu biar limitnya gk cepet abis 
...
var waktoo = date.getHours();
		...
		switch(bulan1) {
    	    ...
    	    if(time2 < "05:00:00"){
				...
				mess = {
					wait: 'Proses Ngab..',
					limit: 'Limit kamu habis Ngab , Silahkan beli dengan #buylimit <nominal> atau minta gift dari user lain',
...
Minat? Pm wa.me/${owner_number}`,
					ban: 'Maaf Ngab Nomor Kamu Sudah Di Banned!',
					error: 'Error Ngab, Mungkin Lagi Diperbaiki',
					errorLink: 'Link Error Ngab',
					succes: '```[ ✓ ]``` Sukses Ngab',
					only: {
						personal: 'Only Private Chat Ngab',
						admin: 'Khusus Admin Group Ngab',
						group: 'Khusus Dalam Grup Ngab',
						 prem: 'Khusus User Premium Ngab',
						owner: 'Khusus Owner Bot Ngab',
						 }
					} 
...
*🎯 Xp : ${getLevelingXp(sender)}*`)
						...
						case 'giftlimit': case 'givelimit':
									if (isPremium) return reply(`User Premium Tidak Bisa Melakukan Gift Limit Ngab`)
									if (!isGroup) return reply(mess.only.group)
									if (!q.includes('|')) return  reply(`Contoh ${prefix + command} _@user | Nominal_`)
									const tujuan = q.substring(0, q.indexOf('|') - 1)
									const jumblah = q.substring(q.lastIndexOf('|') + 1)
									if(isNaN(jumblah)) return await reply('Jumlah Harus Berupa Angka Ngab')
...
*💹 Sisa Limit : ${getLimit(sender, limitawal, limit)} / ${limitawal}*`)
									...
									break
						case 'playstore': 
									if(!q) return reply('Mau cari apa Ngab')
									reply(mess.wait)
...
_Tunggu Proses Upload Media_`
						...
						case 'wiki': case 'wikipedia':
									if (args.length < 1) return reply(' Yang Mau Di Cari Apa Ngab? ')
									reply(mess.wait)
									teks = args.join(' ')
						...
						case 'asupan':{
									...
									Zeeone.sendMessage(from, p, video, { quoted:  Ofc, mimetype: 'video/mp4', caption: 'Asupan Ngab'})
									limitAdd(sender, limit)
									}
									break
						...
						case 'afk':
									if (!isGroup) return reply(mess.only.group)
									if (isAfkOn) return reply('Kamu dalam mode afk Ngab')
...
Alasan : ${reason}`, [sender], true)
						...
						case 'join':case 'joingc':
									if (!q) return reply('Linknya?')
									if (!Ofc.key.fromMe && !isOwner) return reply(mess.only.owner)
										if (!isUrl(args[0]) && !args[0].includes('https://chat.whatsapp.com/')) return reply('Linknya Invalid Ngab')
						...
						case 'setnamabot':{
									...
									.then((res) => reply('Sukses Ngab'))
									.catch((err) => reply('Eror Ngab'))
									 }
									break
						case 'setbiobot':{
									 if (!isOwner && !Ofc.key.fromMe) return reply(mess.only.owner)
									...
									if (isQuotedImage) {
										...
										.catch((err) => reply(jsonformat(err)))
											...
											exec(`ffmpeg -i ${media} ${ran}`, (err) => {
												fs.unlinkSync(media)
												if (err) return reply('Error Ngab')
						...
						case 'cekprem': case 'cekpremium': case 'premiumcek':
									...
									reply(`${isOwner?'Unlimited Prem (My Ngab)':premiumnya}`)
									break
...
💹 *EXPIRE :* ${cekvip.days} day(s) ${cekvip.hours} hour(s) ${cekvip.minutes} minute(s)`
						...
						case 'antibadword': case 'nobadword':
									...
									if (args[0] == 'on'){
										if (isBadword) return reply(`Udah aktif Ngab`)
						...
						case 'addbadword':
									...
									if (Ofc.message.extendedTextMessage === undefined || Ofc.message.extendedTextMessage === null) {
										if (isKasar(args[0].toLowerCase(), badword)) return reply(`Cari nama lain Ngab`)
...
\`\`\`${groupDesc}\`\`\``
						...
						case 'caripesan': case 'searchmessage':
									...
									if(!q)return reply('Pesannya apa Ngab?')
									reply('Otw Ngab')
						...
						case 'tomp3': case 'tomusic': case 'toaudio':
									if (isLimit(sender, isPremium, isOwner, limitawal, limit)) return reply(mess.limit)
									if (!isQuotedVideo) return reply(`Reply video nya Ngab`)
									reply(mess.wait)
						...
						case 'toptt': case 'tovn':
									if (isLimit(sender, isPremium, isOwner, limitawal, limit)) return reply(mess.limit)
									if (!isQuotedAudio) return reply(`Reply Audionya Ngab`)
									reply(mess.wait)
									var encmedia = JSON.parse(JSON.stringify(Ofc).replace('quotedM','m')).message.extendedTextMessage.contextInfo
									var media = await Zeeone.downloadAndSaveMediaMessage(encmedia)
									var ran = getRandom('.mp4')
						...
						case 'toimg': 
									if (isLimit(sender, isPremium, isOwner, limitawal, limit)) return reply(mess.limit)
									reply(mess.wait)
									if (!isQuotedSticker) return reply('Reply stiker Ngab')
									if ((isMedia && !Ofc.message.videoMessage || isQuotedImage || isQuotedSticker && args.length == 0)) {
										...
										exec(`ffmpeg -i ${media} ${ran}`, (err) => {
											...
											buffer = fs.readFileSync(ran)
											Zeeone.sendMessage(from, buffer, image, {thumbnail : thumbnail, caption:'Sukses Ngab',quoted:Ofc})
											fs.unlinkSync(ran)
											})
										} else {
											reply(`Reply Stiker Nya Ngab`)
										}
										limitAdd(sender, limit)
									break
						case 'togif':
									...
									Zeeone.sendMessage(from, Frd, video, { mimetype: 'video/gif', caption: 'Sukses Ngab', quoted: Ofc })
									limitAdd(sender, limit)
									break
						case 'tovideo': case 'tomp4':
									if (isLimit(sender, isPremium, isOwner, limitawal, limit)) return reply(mess.limit)
									if (!isQuotedSticker) return reply('Reply stiker gif Ngab')
									reply(mess.wait)
									if ((isMedia && !Ofc.message.videoMessage || isQuotedSticker) && args.length == 0) {
										...
										webp2mp4File(owgi).then(res=>{
											Zeeone.sendMessage(from, res.result, video, { mimetype: Mimetype.mp4, filename: 'tovideo.mp4',caption: 'Sukses Ngab', quoted: Ofc})
											})
										}else {
											reply('reply stiker')
										}
...
let qqppp = [{
                ...
                sendButLocation(from, teks, `Specstor™© | Allright Reserved.`, thumbnail, qqppp, {contextInfo: { mentionedJid: [nupe]}})
...
break
						...
						case 'nuliskiri':{
									...
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `Sudah Ngab.`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'nuliskanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `Sudah Ngab.`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'foliokiri':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `Sudah Ngab.`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'foliokanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Zeeone.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: Ofc, caption: `Sudah Ngab.`})
										limitAdd(sender, limit)
									})
									}
									break
						...
						case 'getnum': case 'getnomor': case 'getnumber':
									if (!isGroup) return reply(mess.only.group)
									if (Ofc.message.extendedTextMessage === undefined || Ofc.message.extendedTextMessage === null) return reply('Reply Pesan Target Ngab')
									mentioned = Ofc.message.extendedTextMessage.contextInfo.mentionedJid[0]
						...
						case 'addvn': case 'addaudio': case 'addmusic':
									...
									if (!isQuotedAudio) return reply('Reply Audionya Ngab')
									if (!q) return reply('Nama Audionya Apa Ngab?')
						...
						case 'delaudio': case 'delvn': case 'deletevn':
									if (!Ofc.key.fromMe && !isOwner && !isPremium) return reply(mess.only.prem)
									if (!q) return reply(`Nama Audio Yang Mau Di Hapus Apa Ngab?`)
									try {
						...
						case 'addimage': case 'addfoto': case 'addphoto':
									if (!Ofc.key.fromMe && !isOwner && !isPremium) return reply(mess.only.prem)
									if (!isQuotedImage) return reply('Reply Photonya Ngab')
									if (!q) return reply('Nama Photonya Apa Ngab?')
						...
						case 'delimage': case 'deleteimage': case 'delfoto': case 'delphoto':
									if (!Ofc.key.fromMe && !isOwner  && !isPremium) return reply(mess.only.prem)
									if (!q) return reply(`Nama Sticker Yang Mau Di Hapus Apa Ngab?`)
									try {
						...
						case 'addvideo': case 'addvid':
									...
									if (!q) return reply('Nama Videonya Apa Ngab?')
						...
						case 'delvideo': case 'deletevideo':
									if (!Ofc.key.fromMe && !isOwner  && !isPremium) return reply(mess.only.prem)
									if (!q) return reply(`Nama Video Yang Mau Di Hapus Apa Ngab?`)
									try {
						...
						case 'addstik': case 'addstiker': case 'addsticker':
									if (!Ofc.key.fromMe && !isOwner && !isPremium) return reply(mess.only.prem)
									if (!isQuotedSticker) return reply('Reply stiker nya Ngab')
									if (!q) return reply('Nama Sticker Nya Apa Ngab?')
						...
						case 'jadian':
									...
									break
						case 'naruto':
									if (!q) return reply('Teksnya Ngab?')
									reply(mess.wait)
									pNaruto(`${q}`)
									.then(res => {
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Specstor :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
